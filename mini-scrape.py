...
def get_sector_news(sector: str) -> str:
    ...
    print(f"Getting news for sector: {sector}")
...
def get_market_news(market: str) -> str:
    print(f"Getting news for market: {market}")
    news_scrape = aci.functions.execute(
            "FIRECRAWL__SCRAPE",
            {
                "body": {
...
def generate_podcast(text: str) -> str:
    ...
    unfiltered = podcast_resp.choices[0].message.content
...
if __name__ == "__main__": 
    ...
    if isinstance(sectors, str):
        ...
        sectors = sectors.strip("[]").replace("'", "").replace('"', '').split(", ")
        sectors = [s.strip() for s in sectors if s.strip()]
        markets = markets.strip("[]").replace("'", "").replace('"', '').split(", ")
    ...
    # print(sum)
    podcast_script = generate_podcast(sum)
